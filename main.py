...
#Mediapipe Hand Landmarks, Path Movements og Path Lengths
import cv2
...
def tracking_and_path(video):
    ...
    # Begin VideoCapture object and read the first frame
    cap = cv2.VideoCapture(video) #Video.mp4
    ret, frame = cap.read()
    #Begin Hand Tracking Module
    with mp_hands.Hands(static_image_mode=False,
        ...
        while ret:
            ...
            #Extract hand landmarks & drawing specs from Mediapipe results
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    mp_drawing.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)

                    # Extract the position of the wrist (landmark 0) in Mediapipe code
                    x, y = int(hand_landmarks.landmark[0].x * frame.shape[1]), \
                           int(hand_landmarks.landmark[0].y * frame.shape[0])

                    # Wrist position to the corresponding path list
            ...
            #Movement path length for each wrist
            if len(path_left) > 1:
            ...
            #Draw the movement path of both wrists in a separate window, that can be resized
